...
graph TB
    ...
    SSM["SSM Port Forwarding<br/>localhost:8080"]
...
```
浏览器 (Phaser.js)
    ↓ WebSocket + REST (HTTP:8080, SSM Port Forwarding)
...
部署完成后通过 SSM 端口转发访问 EC2 上的 Game Server（EC2 在私有子网，无公网 IP）。

**获取 EC2 实例 ID**:

```bash
# 从 CloudFormation 输出获取 ASG 名称，再获取实例 ID
...
然后浏览器打开 http://localhost:8080 即可访问游戏。
